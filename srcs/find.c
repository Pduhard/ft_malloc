...
// int     check_in_block_list(void *ptr, t_blockhdr *block_lst,
//     t_heaphdr **heap_addr, t_blockhdr **block_addr)
// {
    
// }

int     check_in_heap_list(void *ptr, t_heaphdr *heap_lst, t_heaphdr **heap_addr, t_blockhdr **block_addr)
{
    ...
    while (heap_lst)
    {
        if (ptr > (void *)heap_lst && ptr < (void *)heap_lst + heap_lst->size)
        {
            ...
            // if (!check_in_block_list(ptr, , heap_addr, block_addr))
            // {
            ft_putstr("wrong addr error\n");
            // }
            // else
        }
