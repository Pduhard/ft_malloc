...
void        create_new_block(t_blockhdr *new_block, size_t size,
    e_state state, t_blockhdr *prev)
{
    write(2, "in new blck\n", sizeof("in new blck\n"));
    new_block->next = NULL;
    new_block->next = prev;
...
t_blockhdr  *create_free_space(size_t size)
{
    ...
    if (new_heap->heaptype != HP_LARGE)
    {
        create_new_block((t_blockhdr *)((void *)(new_heap + 1) + sizeof(t_blockhdr) + size),
            ...
            ST_FREE,
            (void *)(new_heap + 1));
        // if (new_heap->size - size - sizeof(t_blockhdr) - sizeof(t_heaphdr) == (size_t)18446744073709551600)
        //     exit(0);
...
t_blockhdr  *check_heap_space(size_t size, t_heaphdr *curr)
{
    ...
    while (block)
    {
        ...
        {
            update_block(block, size, prev_state, ST_USE);
...
t_blockhdr    *find_free_space(size_t size, e_heaptype type)
{
    t_blockhdr   *block;
    t_heaphdr    *curr;
    ...
    while (curr)
    {
       ...
       write(2, "\n", 1);
        if (curr->type == type && curr->free_space >= size
            && (block = check_heap_space(size, curr)))
            return (block);
        write(1, "heap_space out\n", 15);
        curr = curr->next;
    }
...
void *malloc(size_t size)
{
    ...
    type = s
    write(1, "malloc in\n", 10);
    if (!(block = find_free_space(size, type)))
    {
        write(2, "malloc error\n", 13);
        return ((void *)-1);
    }
